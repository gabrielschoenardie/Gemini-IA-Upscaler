      ...
      const convertToJpg = (base64Str: string, mime: string): Promise<string> => {
        return new Promise((resolve) => {
          ...
          img.onload = () => {
            ...
            if (!ctx) return resolve(`data:${mime};base64,${base64Str}`);
            ctx.fillStyle = '#FFFFFF';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            ctx.drawImage(img, 0, 0);

            // POST-PROCESSING: Premium Convolution Sharpening for Professional Mode
            // This legally mathematically destroys VAE blur by shifting edge contrast local pixels.
            if (outputQuality === 'Professional' && tier === 'Pro') {
                try {
                    ...
                    for (let y = 1; y < h - 1; y++) {
                        for (let x = 1; x < w - 1; x++) {
                            ...
                            for (let c = 0; c < 3; c++) { // RGB channels
                                const target = 5 * src[i + c] - src[up + c] - src[down + c] - src[left + c] - src[right + c];
                                data[i + c] = src[i + c] + (target - src[i + c]) * amount;
                            }
                        }
                    }
                    ctx.putImageData(imgData, 0, 0);
                } catch (e) {
                    console.warn("Post-processing sharpening skipped due to canvas size or memory limits.");
                }
            }

            resolve(canvas.toDataURL('image/jpeg', 1.0)); // 100% Quality JPG
          };
          img.onerror = () => resolve(`data:${mime};base64,${base64Str}`);
